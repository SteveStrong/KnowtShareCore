...
namespace KnowtShareCore
{
    public class ShapeHub : Hub
    {
        ...
        public override Task OnConnectedAsync()
        {
            ...
            var result = Clients.All.InvokeAsync("clientCountChanged", _connections.Count, "connected", this.GroupCount.Count);
        ...
        public Task JoinSessionGroup(string sessionKey)
        {
            ...
            return Groups.AddAsync(Context.ConnectionId, sessionKey);
        }
        ...
        public Task LeaveSessionGroup(string sessionKey)
        {
            var count = GroupCount.GetOrAdd(sessionKey, 0);
            GroupCount.TryUpdate(sessionKey, count - 1, count);
            return Groups.RemoveAsync(Context.ConnectionId, sessionKey);
        }
        ...
        {
            Clients.All.InvokeAsync("receivePing", sessionKey, sender, senderId);
        }

        public void SendMessage(string sessionKey, string sender, string senderId, string textMessage)
        {
            Clients.All.InvokeAsync("receiveMessage", sessionKey, sender, senderId);
        }
